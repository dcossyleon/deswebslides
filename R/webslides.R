#' Convert to a webslides presentation
#'
#' Trying out webslides.
#'
...
#' }
#' @export
webslides_presentation <- function(incremental = FALSE,
                               duration = NULL,
                               footer = NULL,
                               font_adjustment = 0,
                               fig_width = 8,
  ...
  if (identical(template, "default"))
    template <- pkg_file("rmd/webslides/default.html")
  if (!is.null(template))
    args <- c(args, "--template", pandoc_path_arg(template))

  # html dependency for slidy
  extra_dependencies <- append(extra_dependencies,
                               list(html_dependency_webslides()))
  ...
  pre_processor <- function(metadata, input_file, runtime, knit_meta, files_dir,
    ...
    # highlight
    #args <- c(args, pandoc_highlight_args(highlight, default = "kate"))
...
html_dependency_webslides <- function() {
  htmlDependency(
    name = "webslides",
    version = "2",
    src = pkg_file("rmd/webslides/Webslides2"),
    script = c(
